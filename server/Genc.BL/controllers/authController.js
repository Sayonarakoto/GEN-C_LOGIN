...
const Faculty = require('../../Genc.DAL/models/Faculty');
const Student = require('../../Genc.DAL/models/student');
const Security = require('../../Genc.DAL/models/security');
...
exports.switchRole = async (req, res, next) => {
  try {
    const { targetRole, targetDepartment } = req.body;
    const userId = req.user?.id;

    if (!userId) {
      return res.status(401).json({ success: false, message: 'Unauthorized. Valid session token required.' });
    }
    ...
    const availableRoles = (user.roles && user.roles.length > 0) ? user.roles : [user.role || 'student'];
    const desiredRole = (targetRole || '').toLowerCase();

    if (!availableRoles.includes(desiredRole)) {
      return res.status(403).json({ success: false, message: `Access denied. Role ${targetRole} is not assigned to your user account.` });
    }

    user.active_role = desiredRole;
    if (targetDepartment) {
      user.department = targetDepartment.toUpperCase();
    }
    await user.save();

    const token = user.getSignedJwtToken();
    const navigationResult = await navigationService.getNavigationForRole(desiredRole);

    return res.status(200).json({
      success: true,
      message: `Successfully switched active role to ${desiredRole}`,
      token,
      user: {
        id: user._id,
        role: desiredRole,
    ...
    return res.status(500).json({ success: false, message: 'Server error switching active role.' });
  }
};
